# Phumza Nkosi — Data Analytics Portfolio

BSc Honours in Biostatistics (NQF Level 8) | BSc Health Sciences in Health Systems Science (NQF Level 7)

Gauteng | comfort10nkosi@gmail.com | (+27) 71 553 1293

---

## About This Portfolio

This portfolio documents a self-directed upskilling journey from health sciences graduate to data analyst — built while actively seeking employment. Every project uses real South African data, targets a specific employer sector, and demonstrates end-to-end analytical capability from raw data to insight.

The projects span health, financial services, and policy domains — reflecting both my academic background and my intention to work across sectors as a cross-disciplinary data analyst.

---

## Tools & Technologies

| Tool | Proficiency | Used In |
|---|---|---|
| SQL (T-SQL / SSMS) | Intermediate — Advanced | Projects 2, 5 |
| Microsoft Excel (Advanced) | Confident | Projects 1, 4 |
| Power BI | Confident | Projects 3, 5 |
| Python — pandas, matplotlib | Basic | None yet |
| GitHub | Version control | All projects |

---

## Projects

| # | Project | Domain | Tools | Status |
|---|---|---|---|---|
| 1 | SA NCD Trend Analysis — Diabetes & Hypertension | Health | Excel | ✅ Complete |
| 2 | SA NCD SQL Analysis — Diabetes & Hypertension | Health | SQL Server | ✅ Complete |
| 3 | SA NCD Power BI Dashboard | Health | Power BI | ✅ Complete |
| 4 | SA NHI Provincial Readiness Scorecard | Health Policy | Excel | ✅ Complete |
| 5 | SA Banking Sector Post-COVID Analysis | Financial Services | SQL Server, Power BI | ✅ Complete |

---

## Key Themes Across Projects

**Health Systems & Policy** — Projects 1–4 engage directly with South Africa's public health data, NHI implementation context, and National Strategic Plan 2030 targets. Relevant to SAMRC, NICD, HSRC, WHO AFRO, health NGOs, and government health departments.

**Financial Services Analytics** — Project 5 uses South African Reserve Bank authoritative data to analyse banking sector performance. Relevant to banks, asset managers, and financial consulting firms.

**Cross-Sector Analytical Thinking** — All projects follow the same rigorous approach: real data, structured analysis, documented methodology, and plain-language findings targeted at non-technical stakeholders.

---

## Certifications

- DHIS2 Fundamentals — DHIS Online Academy, May 2026
- SQL Fundamentals for Data Analysts — Corporate Finance Institute (Coursera), Feb 2026
- Work Smarter with Microsoft Excel — Microsoft (Coursera), Nov 2025
- Creating Compelling Reports — Cisco Networking Academy, Dec 2025

**In Progress:**
- Data Science Essentials with Python — Cisco Networking Academy
- Power BI Fundamentals — CFI (Coursera)

---

## Contact

📧 comfort10nkosi@gmail.com | 📞 (+27) 71 553 1293
